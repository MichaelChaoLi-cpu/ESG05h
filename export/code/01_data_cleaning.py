...
rate_raw  = pd.read_excel(RAW / "Japan_interest_rate_annual.xlsx",
                          sheet_name="Sheet1")
fund_raw  = pd.read_excel(RAW / "annual_financial_raw.xlsx")
ibes_raw  = pd.read_excel(RAW / "IBES_monthly_raw.xlsx")
...
print(f"  Fundamentals    : {fund_raw.shape}")
print(f"  IBES monthly    : {ibes_raw.shape}")
...
# ── 7. Clean fundamentals ────────────────────────────────────────────────────

fund = fund_raw.copy()
fund["stock_code"] = fund["RIC"].str.replace(r"\.[A-Z]+$", "", regex=True)
fund["year"] = fund["PeriodEndDate"].dt.year

# When a company changed fiscal year, two period-ends can fall in the same
# calendar year.  Keep the row with the most complete data; break ties by
# taking the later period-end date.
fund["_nonnull"] = fund[
    ["ROA_Pct", "LTDebt_Assets_Pct", "Debt_Equity_Pct",
     "TotalDebt", "TotalEquity", "TotalAssets",
     "RnD", "TotalRevenue", "RnD_Share_Pct"]
].notna().sum(axis=1)
fund = (
    fund.sort_values(["_nonnull", "PeriodEndDate"], ascending=[False, False])
    .drop_duplicates(subset=["stock_code", "year"], keep="first")
    .drop(columns=["RIC", "PeriodEndDate", "_nonnull"])
)
fund = fund.rename(columns={
    "ROA_Pct":           "roa_pct",
    "LTDebt_Assets_Pct": "ltdebt_assets_pct",
    "Debt_Equity_Pct":   "debt_equity_pct",
    "TotalDebt":         "total_debt",
    "TotalEquity":       "total_equity",
    "TotalAssets":       "total_assets",
    "RnD":               "rnd",
    "TotalRevenue":      "total_revenue",
    "RnD_Share_Pct":     "rnd_share_pct",
})
fund = fund[["stock_code", "year",
             "roa_pct", "ltdebt_assets_pct", "debt_equity_pct",
             "total_debt", "total_equity", "total_assets",
             "rnd", "total_revenue", "rnd_share_pct"]]

# ── 8. Clean IBES (monthly → annual) ─────────────────────────────────────────

ibes = ibes_raw.copy()
ibes["stock_code"] = ibes["RIC"].str.replace(r"\.[A-Z]+$", "", regex=True)
ibes["CalcDate"] = pd.to_datetime(ibes["CalcDate"])

# Take the December month-end snapshot as the annual consensus value.
# All 16,972 RIC-Year pairs carry exactly one December row, so a simple
# filter produces a clean 1:1 mapping to panel (stock_code, year).
ibes = ibes[ibes["CalcDate"].dt.month == 12].copy()
ibes = ibes.rename(columns={
    "Year":                "year",
    "EPS_Mean":            "ibes_eps_mean",
    "Revenue_Mean":        "ibes_rev_mean",
    "Recommendation_Mean": "ibes_rec_mean",
    "PriceTarget_Mean":    "ibes_pt_mean",
})
ibes = ibes[["stock_code", "year",
             "ibes_eps_mean", "ibes_rev_mean",
             "ibes_rec_mean", "ibes_pt_mean"]]
...
panel = panel.merge(fund,  on=["stock_code", "year"], how="left")
panel = panel.merge(ibes,  on=["stock_code", "year"], how="left")

# ── 10. Construct analysis variables ─────────────────────────────────────────
...
# Fundamentals and IBES lags (t-1), for use as predetermined controls
fund_ibes_vars = [
    "roa_pct", "ltdebt_assets_pct", "debt_equity_pct",
    "total_debt", "total_equity", "total_assets",
    "rnd", "total_revenue", "rnd_share_pct",
...
fund_cols    = ["roa_pct", "ltdebt_assets_pct", "debt_equity_pct",
                "total_debt", "total_equity", "total_assets",
                "rnd", "total_revenue", "rnd_share_pct"]
ibes_cols    = ["ibes_eps_mean", "ibes_rev_mean", "ibes_rec_mean", "ibes_pt_mean"]
_esg_seen = set()
esg_idx_cols = []
...
ordered_cols = (id_cols + gics_cols + outcome_cols + macro_cols
                + fund_cols + ibes_cols + esg_idx_cols + theme_cols)
...
missing_roa  = panel.roa_pct.isna().sum()
missing_rnd  = panel.rnd_share_pct.isna().sum()
missing_ibes = panel.ibes_rec_mean.isna().sum()
print(f"\nMissing market cap   : {missing_mc} obs ({missing_mc / len(panel):.1%})")
print(f"Missing GICS sector  : {missing_gics} obs ({missing_gics / len(panel):.1%})")
print(f"Missing ROA          : {missing_roa} obs ({missing_roa / len(panel):.1%})")
print(f"Missing RnD share    : {missing_rnd} obs ({missing_rnd / len(panel):.1%})")
print(f"Missing IBES rec     : {missing_ibes} obs ({missing_ibes / len(panel):.1%})")
